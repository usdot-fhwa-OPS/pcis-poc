...
import {
  ...
  TerminalBookingsCompleted,
} from '../components/terminal-bookings/terminal-bookings-table.tsx'
import {
  TransportationBookingsTableUpcoming,
  TransportationBookingsTableCompleted,
  TransportationBookingsTableOngoing,
} from '../components/transportation_bookings/transportation-bookings-table.tsx'
import {
  BcoBookingsTableUpcoming,
  BcoBookingsTableCompleted,
  BcoBookingsTableOngoing,
} from '../components/bco_bookings/bco-bookings-table.tsx'
...
function RouteComponent() {
    ...
    async function getUserSession() {
      if (user) {
        try {
          ...
          console.log(
            'User session:',
            session.tokens?.idToken?.toString() ?? 'No session found',
          )
          console.log(
            'User access token:',
            session.tokens?.accessToken.toString() ?? 'No access token found',
          )
        } catch (error) {
  ...
  const [Transportation_CompletedData, setTransportation_CompletedData] =
    useState<TransOperatorCompletedBookings[]>([])
  ...
  const [Terminal_CompletedData, setTerminal_CompletedData] = useState<
    TermOperatorCompletedBookings[]
  >([])
  ...
  const [bcoUpcomingBookings, setBcoUpcomingBookings] = useState<
    BCOUpcomingBookings[]
  >([])
  ...
  const [bcocompletedBookings, setBcoCompletedBookings] = useState<
    BCOCompletedBooking[]
  >([])
  ...
  const [transOpUpcomingBookings, setTransOpUpcomingBookings] = useState<
    TransOpUpcomingBookings[]
  >([])
        ...
        const { data: cargo } = await client.models.Container.list({
          filter: {
            and: [
              ...
              {
                bookingStatus: {
                  eq: 'Pending Transportation Operator Approval',
                },
              },
            ],
          },
  ...
  const [transOpOngoingBookings, setTransOpOngoingBookings] = useState<
    TransOpOngoingBookings[]
  >([])
  ...
  async function assignTransOp(
    ...
    try {
      const { data: assignTransportationOp } =
  ...
  const [terminalOpModifiedBookings, setTerminalOpModifiedBookings] = useState<
    TerminalOpModifiedBookings[]
  >([])
  ...
  //Fetch the data from the database
  const [terminalopBookingongoing, set_terminal_ongoing] = useState<
    TerminalOPOngoingBookings[]
  >([])
        ...
        <Tabs defaultValue="requested" className="">
          ...
          <div>
            ...
            <TabsContent value="modification">
              <TerminalBookingsTable
                data={terminalOpModifiedBookings}
                status="Modified"
                meta={{ updateBooking }}
              />
            </TabsContent>
            <TabsContent value="ongoing">
              <TerminalBookingsTable
                data={terminalopBookingongoing}
                status="Ongoing"
                meta={{ updateBooking, markBookingLate }}
              />
            </TabsContent>
            <TabsContent value="completed">
              <TerminalBookingsCompleted
                data={Terminal_CompletedData}
                status="Completed"
                meta={{ updateBooking }}
              />
            </TabsContent>
